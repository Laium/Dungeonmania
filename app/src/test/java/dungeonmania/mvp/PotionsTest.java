...
public class PotionsTest {
    ...
    @Test
    @Tag("6-9")
    @DisplayName("Test fleeing enemies move as far away as possible")
    public void invincibilityPotionFleeFar() throws InvalidActionException {
        DungeonManiaController dmc = new DungeonManiaController();
        DungeonResponse res = dmc.newGame("d_potionsTest_invincibilityFleeFar", "c_potionsTest_invincibilityFleeFar");
        ...
        assertEquals(new Position(6, 2), zombiePos); // Zombie should be at the furthest point of its box
        assertEquals(new Position(4, 7), mercPos); // Mercenary is trapped so can't move

    }

    @Test
    @Tag("6-9")
    @DisplayName("Test invincibility potions have no effect on allied mercenaries")
    public void invincibilityPotionAllies() throws InvalidActionException {
        DungeonManiaController dmc = new DungeonManiaController();
        DungeonResponse res = dmc.newGame("d_potionsTest_invincibilityAllies", "c_potionsTest_invincibilityAllies");
        ...
        // Player and mercenary should be adjacent
        assertEquals(new Position(3, 4), TestUtils.getPlayerPos(res));
        assertEquals(new Position(4, 4), TestUtils.getEntityById(res, mercId).get().getPosition());

        // Move back left and use potion
        res = dmc.tick(Direction.LEFT);
        assertEquals(new Position(2, 4), TestUtils.getPlayerPos(res));
        assertEquals(new Position(3, 4), TestUtils.getEntityById(res, mercId).get().getPosition());

        res = dmc.tick(TestUtils.getFirstItemId(res, "invincibility_potion"));

        // Ally should be following
        assertEquals(new Position(3, 4), TestUtils.getEntityById(res, mercId).get().getPosition());
        res = dmc.tick(Direction.LEFT);
        assertEquals(new Position(2, 4), TestUtils.getEntityById(res, mercId).get().getPosition());
